...
bool Explorer::ignored(const std::string& name) {
    return name == ".git" || name == "node_modules" || name == "__pycache__" || name == ".DS_Store";
}
...
Explorer::Node* Explorer::find(Node& node, const std::filesystem::path& path) {
    if (node.path == path || node.path.lexically_normal() == path.lexically_normal()) {
        return &node;
    }
    std::error_code a_ec;
    std::error_code b_ec;
    std::error_code eq_ec;
    if (std::filesystem::exists(node.path, a_ec) && !a_ec && std::filesystem::exists(path, b_ec) &&
        !b_ec && std::filesystem::equivalent(node.path, path, eq_ec) && !eq_ec) {
        return &node;
    }
