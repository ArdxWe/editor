...
App::App(const std::filesystem::path& path) : context_(SDL_INIT_VIDEO) {
    ...
    const float scale = win.h > 0 ? static_cast<float>(out.h) / static_cast<float>(win.h) : 1.f;
...
void App::run() {
    ...
    while (running) {
        ...
        bool got_event = needs_redraw_ ? SDL_PollEvent(&event) : SDL_WaitEventTimeout(&event, wait_ms);
        while (got_event) {
...
const Texture* App::cached_texture(const char* text, SDL_Color fg, SDL_Color bg) {
    if (!text || !*text) {
        return nullptr;
    }
    ...
    for (auto& entry : text_cache_) {
        if (entry->fg == packed_fg && entry->bg == packed_bg && entry->text == text) {
    ...
    text_cache_.push_back(std::make_unique<CachedText>(
        CachedText{text, packed_fg, packed_bg, cache_stamp_, Texture{renderer_, surface}}));
...
bool App::open_file(const std::filesystem::path& path) {
    ...
    if (doc_.has_file() && std::filesystem::equivalent(doc_.path(), path, ec) && !ec) {
        return true;
    }
    if (doc_.dirty()) {
...
void App::handle_event(const SDL_Event& event) {
    ...
    if (event.type == SDL_EVENT_TEXT_INPUT) {
        if ((SDL_GetModState() & (SDL_KMOD_CTRL | SDL_KMOD_GUI | SDL_KMOD_ALT)) != 0) {
            return;
        }
        if (term_focus_) {
    ...
    if (event.type == SDL_EVENT_MOUSE_WHEEL) {
        ...
        if (event.wheel.mouse_x < static_cast<float>(side) &&
            ...
            const int max_scroll = std::max(0, static_cast<int>(doc_.lines().size()) - 1);
    ...
    if (event.type == SDL_EVENT_MOUSE_BUTTON_DOWN && event.button.button == SDL_BUTTON_LEFT) {
        ...
        if (event.button.x < static_cast<float>(side)) {
            const int row = tree_scroll_ + static_cast<int>((event.button.y - kPad) / line_h);
            if (row < 0 || row >= static_cast<int>(explorer_.rows().size())) {
                return;
            }
            const TreeRow& item = explorer_.rows()[static_cast<std::size_t>(row)];
            std::error_code dir_ec;
            const bool is_dir = item.is_dir || std::filesystem::is_directory(item.path, dir_ec);
            if (is_dir) {
        ...
        const int row = scroll_ + static_cast<int>((event.button.y - kPad) / line_h);
        const auto& lines = doc_.lines();
        int target = std::clamp(row, 0, static_cast<int>(lines.size()) - 1);
        const int gutter = gutter_width();
        const int col = byte_at_x(
            lines[static_cast<std::size_t>(target)],
...
void App::handle_terminal_key(const SDL_Event& event) {
    ...
    const bool ctrl = (event.key.mod & SDL_KMOD_CTRL) && !(event.key.mod & SDL_KMOD_GUI);
...
int App::terminal_top() const { return status_bar_top() - terminal_panel_height(); }

int App::content_bottom() const { return term_open_ ? terminal_top() : status_bar_top(); }
...
App::Split App::hit_split(float x, float y) const {
    ...
    if (y >= static_cast<float>(h_line - hit) && y <= static_cast<float>(h_line + hit) &&
...
int App::byte_at_x(const std::string& line, float x) const {
    ...
    int left_w = 0;
    const std::size_t left = font_.fit(line.c_str(), static_cast<int>(x), &left_w);
    if (left >= line.size()) {
...
void App::draw_tree(int line_h, int content_bottom) {
    ...
    const SDL_FRect bar{0.f, 0.f, static_cast<float>(side), static_cast<float>(content_bottom)};
    ...
    for (int i = 0; i < vis; ++i) {
        ...
        if (row.active) {
            renderer_.set_draw_color(kTreeActive.r, kTreeActive.g, kTreeActive.b, 255);
            const SDL_FRect hi{0.f, y - 2.f, static_cast<float>(side), static_cast<float>(line_h)};
            renderer_.fill_rect(hi);
            renderer_.set_draw_color(kAccent.r, kAccent.g, kAccent.b, 255);
            renderer_.fill_rect(SDL_FRect{0.f, y - 2.f, 3.f, static_cast<float>(line_h)});
        }
        const std::string label = tree_label(row);
        if (const Texture* tex = cached_texture(label.c_str(), kTree, row.active ? kTreeActive : kTreeBg)) {
            const SDL_FRect dest{static_cast<float>(kPad), y, tex->width(), tex->height()};
            renderer_.copy(*tex, nullptr, &dest);
        }
    }
    renderer_.set_clip(nullptr);
    stroke_v(renderer_, static_cast<float>(side - 1), 0.f, static_cast<float>(content_bottom));
}
...
void App::draw_editor(int line_h, int content_bottom) {
    ...
    if (!doc_.has_file()) {
        const char* hint = "Open a file from the sidebar";
        if (const Texture* tex = cached_texture(hint, kStatus, kPage)) {
            const SDL_FRect dest{static_cast<float>(left + kPad * 2), static_cast<float>(kPad), tex->width(), tex->height()};
            renderer_.copy(*tex, nullptr, &dest);
        }
        return;
    }
    ...
    for (int i = 0; i < vis; ++i) {
        ...
        if (const Texture* num_tex = cached_texture(num.c_str(), kLineNo, num_bg)) {
            const float nx = static_cast<float>(left + gutter - kPad) - num_tex->width();
        ...
        for (const Token& token : tokens) {
            ...
            if (const Texture* tex = cached_texture(piece.c_str(), token_color(token.kind), text_bg)) {
...
void App::draw_terminal(int line_h, int term_top, int status_y) {
    ...
    const SDL_FRect panel{
        0.f,
        static_cast<float>(term_top),
        static_cast<float>(out.w),
        static_cast<float>(status_y - term_top)};
    renderer_.fill_rect(panel);
    stroke_h(renderer_, 0.f, static_cast<float>(term_top), static_cast<float>(out.w));

    const float tab_y = static_cast<float>(term_top + 4);
    if (const Texture* tab = cached_texture("TERMINAL", term_focus_ ? kAccent : kStatus, kTermBg)) {
        const SDL_FRect dest{static_cast<float>(kPad), tab_y, tab->width(), tab->height()};
    ...
    stroke_h(renderer_, 0.f, static_cast<float>(term_top + tab_h), static_cast<float>(out.w));
    ...
    term_scroll_ = std::clamp(term_scroll_, 0, max_scroll);
    const int first = std::max(0, static_cast<int>(lines.size()) - vis - term_scroll_);
    ...
    for (int i = 0; i < vis; ++i) {
        ...
        if (term_focus_ && index == terminal_.cursor_row() && caret_visible()) {
            const std::size_t col = static_cast<std::size_t>(std::max(0, terminal_.cursor_col()));
            const int prefix_w =
                col == 0 || line.empty() ? 0 : font_.measure(line.c_str(), std::min(col, line.size()));
            const SDL_FRect caret{
                static_cast<float>(kPad + prefix_w),
                y,
                2.f,
                static_cast<float>(line_h)};
            renderer_.set_draw_color(kAccent.r, kAccent.g, kAccent.b, 255);
...
void App::draw() {
    ...
    if (side_hot) {
        renderer_.set_draw_color(kAccent.r, kAccent.g, kAccent.b, 255);
        renderer_.fill_rect(SDL_FRect{static_cast<float>(side - 2), 0.f, 4.f, static_cast<float>(bottom)});
    }
    if (term_hot || term_open_) {
        const float hy = static_cast<float>(term_open_ ? terminal_top() : status_y);
        if (term_hot) {
            renderer_.set_draw_color(kAccent.r, kAccent.g, kAccent.b, 255);
            renderer_.fill_rect(SDL_FRect{0.f, hy - 2.f, static_cast<float>(out.w), 4.f});
        } else {
    ...
    const SDL_FRect bar{
        0.f,
        static_cast<float>(status_y - 4),
        static_cast<float>(out.w),
        static_cast<float>(line_h + kPad + 4)};
    renderer_.fill_rect(bar);
    stroke_h(renderer_, 0.f, static_cast<float>(status_y - 4), static_cast<float>(out.w));

    std::string hint;
    if (doc_.has_file()) {
        std::error_code ec;
        auto rel = std::filesystem::relative(doc_.path(), explorer_.root(), ec);
        hint = (!ec && !rel.empty()) ? rel.generic_string() : doc_.path().filename().string();
        hint += "    ";
    }
#ifdef __APPLE__
    hint += "Cmd+S save    Cmd+J terminal";
#else
    hint += "Ctrl+S save    Ctrl+J terminal";
#endif
    if (term_focus_) {
        hint += "    Esc editor";
    }
    if (const Texture* hint_tex = cached_texture(hint.c_str(), kStatus, kStatusBg)) {
        const SDL_FRect dest{
            static_cast<float>(kPad), static_cast<float>(status_y), hint_tex->width(), hint_tex->height()};
