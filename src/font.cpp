...
void Font::attach_cjk_fallback(float ptsize) {
    for (const CjkFont& candidate : cjk_font_candidates) {
        ...
        fallback_ = cjk;
        const int skip = std::max(TTF_GetFontLineSkip(font_), TTF_GetFontLineSkip(cjk));
...
Font::Font(Font&& other) noexcept : font_(other.font_), fallback_(other.fallback_) {
    other.font_ = nullptr;
...
std::size_t Font::fit(const char* text, int max_width, int* measured_width) const {
    int width = 0;
    std::size_t bytes = 0;
