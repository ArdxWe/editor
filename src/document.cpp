...
void Document::backspace() {
    auto& line = lines_[static_cast<std::size_t>(row_)];
    if (col_ > 0) {
        const int prev = static_cast<int>(utf8_prev(line, static_cast<std::size_t>(col_)));
        line.erase(static_cast<std::size_t>(prev), static_cast<std::size_t>(col_ - prev));
        col_ = prev;
        dirty_ = true;
        return;
    }
...
void Document::erase_forward() {
    ...
    if (col_ < static_cast<int>(line.size())) {
        const int next = static_cast<int>(utf8_next(line, static_cast<std::size_t>(col_)));
        line.erase(static_cast<std::size_t>(col_), static_cast<std::size_t>(next - col_));
        dirty_ = true;
        return;
    }
...
void Document::move_left() {
    if (col_ > 0) {
        col_ = static_cast<int>(utf8_prev(lines_[static_cast<std::size_t>(row_)], static_cast<std::size_t>(col_)));
        return;
    }
    if (row_ > 0) {
        --row_;
...
void Document::move_end() { col_ = static_cast<int>(lines_[static_cast<std::size_t>(row_)].size()); }
